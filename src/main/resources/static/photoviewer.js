    ...
    var movable = {
        ...
        movable: function movable(stage, image) {
            ...
            var dragStart = function dragStart(e) {
                ...
                startX = e.type === 'touchstart' ? e.targetTouches[0].pageX : e.clientX;
                startY = e.type === 'touchstart' ? e.targetTouches[0].pageY : e.clientY; // WEI is the difference between image width and height

                WEI = !_this.isRotated ? 0 : (imageWidth - imageHeight) / 2; // Width or height difference can be use to limit image right or top position
                ...
                left = $$1(image).position().left - WEI;
                top = $$1(image).position().top + WEI; // Add grabbing cursor
            ...
            var dragMove = function dragMove(e) {
                ...
                if (isDragging) {
                    ...
                    if (heightDiff > 0) {
                        if (relativeY + top > WEI) {
                            newTop = WEI;
                        } else if (relativeY + top < -heightDiff + WEI) {
                            newTop = -heightDiff + WEI;
                        }
                    } else {
                        newTop = top;
                    } // horizontal limit
                    ...
                    if (widthDiff > 0) {
                        if (relativeX + left > -WEI) {
                            newLeft = -WEI;
                        } else if (relativeX + left < -widthDiff - WEI) {
                            newLeft = -widthDiff - WEI;
                        }
                    } else {
                        newLeft = left;
                    }
    ...
    var resizable = {
        ...
        resizable: function resizable(modal, stage, image, minWidth, minHeight) {
            ...
            var startX = 0,
                ...
                // WEI is the difference between image width and height
                WEI = 0,
                imgWidth = 0,
                imgHeight = 0,
            ...
            var getImageOpts = function getImageOpts(dir, offsetX, offsetY) {
                ...
                var widthDiff = offsetX + modalData.w > minWidth ? stageData.w - imgWidth + offsetX - WEI : minWidth - (modalData.w - stageData.w) - imgWidth - WEI,
                    heightDiff = offsetY + modalData.h > minHeight ? stageData.h - imgHeight + offsetY + WEI : minHeight - (modalData.h - stageData.h) - imgHeight + WEI,
                    widthDiff2 = -offsetX + modalData.w > minWidth ? stageData.w - imgWidth - offsetX - WEI : minWidth - (modalData.w - stageData.w) - imgWidth - WEI,
                    heightDiff2 = -offsetY + modalData.h > minHeight ? stageData.h - imgHeight - offsetY + WEI : minHeight - (modalData.h - stageData.h) - imgHeight + WEI; // Get image position in dragging

                var imgLeft = (widthDiff > 0 ? $$1(image).position().left : $$1(image).position().left < 0 ? $$1(image).position().left : 0) - WEI,
                    imgTop = (heightDiff > 0 ? $$1(image).position().top : $$1(image).position().top < 0 ? $$1(image).position().top : 0) + WEI,
                    imgLeft2 = (widthDiff2 > 0 ? $$1(image).position().left : $$1(image).position().left < 0 ? $$1(image).position().left : 0) - WEI,
                    imgTop2 = (heightDiff2 > 0 ? $$1(image).position().top : $$1(image).position().top < 0 ? $$1(image).position().top : 0) + WEI;
                var opts = {
                    'e': {
                        left: widthDiff >= -WEI ? (widthDiff - WEI) / 2 + 'px' : imgLeft > widthDiff ? imgLeft + 'px' : widthDiff + 'px'
                    },
                    's': {
                        top: heightDiff >= WEI ? (heightDiff + WEI) / 2 + 'px' : imgTop > heightDiff ? imgTop + 'px' : heightDiff + 'px'
                    },
                    'se': {
                        top: heightDiff >= WEI ? (heightDiff + WEI) / 2 + 'px' : imgTop > heightDiff ? imgTop + 'px' : heightDiff + 'px',
                        left: widthDiff >= -WEI ? (widthDiff - WEI) / 2 + 'px' : imgLeft > widthDiff ? imgLeft + 'px' : widthDiff + 'px'
                    ...
                    'w': {
                        left: widthDiff2 >= -WEI ? (widthDiff2 - WEI) / 2 + 'px' : imgLeft2 > widthDiff2 ? imgLeft2 + 'px' : widthDiff2 + 'px'
                    },
                    'n': {
                        top: heightDiff2 >= WEI ? (heightDiff2 + WEI) / 2 + 'px' : imgTop2 > heightDiff2 ? imgTop2 + 'px' : heightDiff2 + 'px'
                    },
                    'nw': {
                        top: heightDiff2 >= WEI ? (heightDiff2 + WEI) / 2 + 'px' : imgTop2 > heightDiff2 ? imgTop2 + 'px' : heightDiff2 + 'px',
                        left: widthDiff2 >= -WEI ? (widthDiff2 - WEI) / 2 + 'px' : imgLeft2 > widthDiff2 ? imgLeft2 + 'px' : widthDiff2 + 'px'
            ...
            var dragStart = function dragStart(dir, e) {
                ...
                WEI = !_this.isRotated ? 0 : (imageData.w - imageData.h) / 2;
        ...
        function () {
            ...
            _proto.zoomTo = function zoomTo(ratio, origin, e) {
                ...
                var newWidth = this.imageData.originalWidth * ratio,
                    ...
                    newTop = origin.y - (origin.y - imgData.y) / imgData.h * newHeight; // WEI is the difference between image new width and new height

                var WEI = !this.isRotated ? 0 : (newWidth - newHeight) / 2,
                ...
                if (imgNewHeight <= stageData.h) {
                    ...
                    newTop = newTop > WEI ? WEI : newTop > offsetY - WEI ? newTop : offsetY - WEI;
                }

                if (imgNewWidth <= stageData.w) {
                    newLeft = (stageData.w - newWidth) / 2;
                } else {
                    newLeft = newLeft > -WEI ? -WEI : newLeft > offsetX + WEI ? newLeft : offsetX + WEI;
